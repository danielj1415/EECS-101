...
int main( int argc, char **argv )
{
	...
	threshold[0] = 132; // choose thresholds values for each image for generate binary images
	threshold[1] = 149;
	threshold[2] = 112;
	...
	for ( k = 0; k < 3; k ++)
	{
		...
		*/
		...
		for(i = 1; i < ROWS - 1; i++)
		{
			...
			{
				gxMax = abs(image[i - 1][j - 1] + 2*image[i][j - 1] + image[i + 1][j - 1] - image[i - 1][j + 1] - 2*image[i][j + 1] - image[i + 1][j + 1]);
				if(gxMax > max[0])
				{
					max[0] = gxMax;
				}
				gyMax = abs(  image[i-1][j-1] + 2*image[i-1][j] + image[i-1][j+1] - image[i+1][j-1] - 2*image[i+1][j] - image[i+1][j+1]);
    			if (gyMax > max[1]) 
				{
      				max[1] = gyMax;
				}
				xMax = abs(  image[i-1][j-1] + 2*image[i][j-1] + image[i+1][j-1] - image[i-1][j+1] - 2*image[i][j+1] - image[i+1][j+1]);
    			yMax = abs(  image[i-1][j-1] + 2*image[i-1][j] + image[i-1][j+1] - image[i+1][j-1] - 2*image[i+1][j] - image[i+1][j+1]);
    			sgmMax = xMax*xMax + yMax*yMax;
				if (sgmMax > max[2])
				{
					max[2] = sgmMax;
				}
	    ...
	    */
		...
		for (i = 1; i < ROWS - 1; i++) 
		{
  			for (j = 1; j < COLUMNS - 1; j++) 
			{
				s = (ximage[i][j] * ximage[i][j] + yimage[i][j] * yimage[i][j]);
				ximage[i][j] = (float)sqrt(s) / max[2] * 255;
  			}
		}
