...
{
	int				i;
	float			t, x, y, z, xprime, yprime, xprime2, yprime2;
	FILE			*fp;
	char			 *ofile;
	...
	for (t = 0.01; t < 10000.01; t += .01)  //change the t range
	{
		...
		float xm = x2;					// xm is x^
		y = y0 + t * b;
		float ym = y0 + t * b;			// ym is y^
		z = z0 + t * c;
		float zm = z0 + t * c;			// zm is z^

		//perspective projection expressions
		xprime = (fprime * x1) / (z0 + t * c);
		yprime = (fprime * (y0 + t * b)) / (z0 + t * c);

		xprime2 = (fprime * x2) / (z0 + t * c);
		yprime2 = (fprime * (y0 + t * b)) / (z0 + t * c);

		//orthographic projection expressions
		/*xprime = x1;
		yprime = y0 + t * b;
		xprime2 = x2;
		yprime2 = y0 + t * b;*/
		...
		plot_logical_point(xprime2, yprime2, image);
	}
	...
	ofile = "hw1testperspective.ras";// this is the name of your output file, modify the name according to the requirements
